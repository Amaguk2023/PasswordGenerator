...
def new_pr_key_pu_key_creation():
    ...
    while not bits:
        try:
            bits = int(input('\nAmount of bits >> '))
            private_key_generation = RSA.generate(
                bits)  # Generating private key (RsaKey object) of key length of 2048 bits
            public_key_generation = private_key_generation.public_key()  # Generating the public key (RsaKey object)
